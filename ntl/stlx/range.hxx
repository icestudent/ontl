/**\file*********************************************************************
 *                                                                     \brief
 *  A minimal std::range<Iter> based on N3350
 *
 ****************************************************************************
 */
#ifndef NTL__STLX_RANGE
#define NTL__STLX_RANGE
#pragma once
...
#if defined(NTL_CXX_TYPEOF) && defined(NTL_CXX_RV)
#include "cassert.hxx"
#include "iterator.hxx"
#include "type_traits.hxx"
#include "utility.hxx"
#define NTL_STLX_RANGE
...
namespace std
{
  ...
  namespace __ { namespace ranged {
    ...
    // ranges. [stmt.ranged]
    template<typename R>
    struct is_range : is_same<typename __::ranged::begin_result<R>::type,
      typename __::ranged::end_result<R>::type> {};

    template<typename R, class C>
    struct is_ranged
    {
      typedef typename remove_cv<typename remove_reference<R>::type>::type U;
      static const bool value = is_range<R>::value 
        && !is_same<C, U>::value
        && !is_same<typename C::value_type, U>::value;
      typedef bool type;
    };
    //template<typename R, class C, class T>
    //struct is_ranged<R,C,T>: integral_constant<bool, is_range<R>::value 
    //  && !is_same<C, typename remove_cv<typename remove_reference<R>::type>::type>::value
    //  && !is_same<T, typename remove_cv<typename remove_reference<R>::type>::type>::value>
    //{};
  ...
  public:
    ...
    constexpr range(R&& r,
      ...
      __::ranged::is_range<R>::value &&
      is_convertible<typename __::ranged::begin_result<R>::type,
      Iterator>::value
      >::type* =0)
      : begin_(__::ranged::adl_begin(r)), end_(__::ranged::adl_end(r)) {}
    ...
    constexpr range(R&& r,
      typename enable_if<is_pointer<Iterator>::value &&
      __::ranged::is_contiguous_range<R>::value &&
      __::ranged::conversion_preserves_array_indexing<
      decltype(&*__::ranged::adl_begin(r)),
      Iterator>::value
      >::type* =0)
      : begin_((__::ranged::adl_begin(r) == __::ranged::adl_end(r) &&
      !is_pointer<decltype(__::ranged::adl_begin(r))>::value)
      ...
      // detectable.
      ? nullptr : &*__::ranged::adl_begin(r)),
      end_(begin_ + (__::ranged::adl_end(r) - __::ranged::adl_begin(r))) {}
  ...
  template<typename Iterator>
  inline constexpr range<Iterator> make_range(Iterator begin, Iterator end)
  {
  ...
  inline constexpr auto make_range(Range&& r,
    typename enable_if<__::ranged::is_range<Range>::value>::type* =0) -> range<decltype(begin(r))>
  {
  ...
  template<typename Range>
  inline constexpr auto make_ptr_range(Range&& r,
    typename enable_if<__::ranged::is_contiguous_range<Range>::value &&
    is_pointer<decltype(&*begin(r))>::value>::type* =0) -> range<decltype(&*begin(r))>
  {
      return range<decltype(&*begin(r))>(r);
  }
