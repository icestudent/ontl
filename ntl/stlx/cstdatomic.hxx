...
#pragma warning(push)
#pragma warning(disable: 4127) // conditional expression is constant: assert(false && "message")
#pragma warning(disable: 4146) // unary minus operator applied to unsigned type, result still unsigned
    ...
    public:
      ...
      void store(type value, memory_order mo = memory_order_seq_cst) volatile
      {
        ...
        case memory_order_release:
          ...
          break;
        default:
          assert(false && "invalid memory order specified");
      ...
      public base<T, true>
    {
      ...
      type fetch_and(type operand, memory_order mo = memory_order_seq_cst) volatile
      {
        ...
        case memory_order_release:
          intrin::_ReadWriteBarrier();
          return rep<type>::cast(atomic::and(val.val, operand));
        case memory_order_acquire:
        case memory_order_consume:
          {const type r = rep<type>::cast(atomic::and(val.val, operand));
          intrin::_ReadWriteBarrier();
          return r;}
        default:
          assert(false && "invalid memory order specified");
      ...
      type fetch_or (type operand, memory_order mo = memory_order_seq_cst) volatile
      {
        ...
        case memory_order_release:
          intrin::_ReadWriteBarrier();
          return rep<type>::cast(atomic::or(val.val, operand));
        case memory_order_acquire:
        case memory_order_consume:
          {const type r = rep<type>::cast(atomic::or(val.val, operand));
          intrin::_ReadWriteBarrier();
          return r;}
        default:
          assert(false && "invalid memory order specified");
      ...
      type fetch_xor(type operand, memory_order mo = memory_order_seq_cst) volatile
      {
        ...
        case memory_order_release:
          intrin::_ReadWriteBarrier();
          return rep<type>::cast(atomic::xor(val.val, operand));
        case memory_order_acquire:
        case memory_order_consume:
          {const type r = rep<type>::cast(atomic::xor(val.val, operand));
          intrin::_ReadWriteBarrier();
          return r;}
        default:
          assert(false && "invalid memory order specified");
