...
namespace std
{
  ...
  *@{*/
  ...
  namespace __
  {
    template<bool f, size_t, class>
    struct get_arg_t { typedef ttl::meta::empty_type type; };

    template<size_t I, class Args>
    struct get_arg_t<true, I, Args>  { typedef typename tuple_element<I, Args>::type type; };
...
#define NTL_DEFINE_MEMFNOP(n, aux) \
  ...
  template<typename R, class T, class Functor, class... Args>
  class mem_fn_t:
    public __::memfun_arity<R, T, tuple<Args...> >
  {
    Functor pmf;
    static const size_t arity = sizeof...(Args);
  public:

    using __::memfun_arity<R, T, tuple<Args...>>::result_type;
    ...
    //result_type operator()(U&& obj) const
    //{
    //  static_assert(arity == 0, "wrong count of arguments");
    //  return func::invoke<result_type>(pmf, typename __::tmap<U&&>::type(forward<U>(obj)));
    //}
#if 1
    template<typename U>
    result_type operator()(U&& obj, Args... args) const
    {
      //typedef typename __::tmap<U&&>::type argU;
      //typedef typename __::tmap<Args...> argT;
      //typedef typename tuple_cat<argU, argT>::type map;
      return __::func::invoke<result_type>(pmf, make_tuple(forward<U>(obj), args...));
    }
#else
