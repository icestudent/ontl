...
namespace ntl { namespace nt {
  ...
  {
    inline void set(error_type errcode)
    {
      ntl::nt::teb::instance().LastErrorValue = errcode;
    }

    inline error_type get()
    {
      return static_cast<error_type>(ntl::nt::teb::instance().LastErrorValue);
    }
    ...
    enum error_type {
      ...
      wsaservice_not_found       = wsa_error_base +  108,
      wsatype_not_found,
      wsa_e_no_more,
      wsa_e_cancelled,

      wsahost_not_found          = wsa_error_base + 1001,
      wsatry_again               = wsa_error_base + 1002,
      wsano_recovery             = wsa_error_base + 1003,
      wsano_data                 = wsa_error_base + 1004,
      qos_receivers              = wsa_error_base + 1005,
      qos_senders                = wsa_error_base + 1006,
      qos_no_senders             = wsa_error_base + 1007,
      qos_no_receivers           = wsa_error_base + 1008,
      qos_request_confirmed      = wsa_error_base + 1009,
      qos_admission_failure      = wsa_error_base + 1010,
      qos_policy_failure         = wsa_error_base + 1011,
      qos_bad_style              = wsa_error_base + 1012,
      qos_bad_object             = wsa_error_base + 1013,
      qos_traffic_ctrl_error     = wsa_error_base + 1014,
      qos_generic_error          = wsa_error_base + 1015,
      qos_eservicetype           = wsa_error_base + 1016,
      qos_eflowspec              = wsa_error_base + 1017,
      qos_eprovspecbuf           = wsa_error_base + 1018,
      qos_efilterstyle           = wsa_error_base + 1019,
      qos_efiltertype            = wsa_error_base + 1020,
      qos_efiltercount           = wsa_error_base + 1021,
      qos_eobjlength             = wsa_error_base + 1022,
      qos_eflowcount             = wsa_error_base + 1023,
      qos_eunkownpsobj           = wsa_error_base + 1024,
      qos_epolicyobj             = wsa_error_base + 1025,
      qos_eflowdesc              = wsa_error_base + 1026,
      qos_epsflowspec            = wsa_error_base + 1027,
      qos_epsfilterspec          = wsa_error_base + 1028,
      qos_esdmodeobj             = wsa_error_base + 1029,
      qos_eshaperateobj          = wsa_error_base + 1030,
      qos_reserved_petype        = wsa_error_base + 1031,
    ...
    public:
      ...
      /** Returns a string naming the error category ("win32") */
      const char *name() const __ntl_nothrow { return "win32"; }

      virtual std::string message(int ev) const
      {
        return ntl::nt::strerror(static_cast<win32error>(ev));
      }

      std::error_condition default_error_condition(int ev) const __ntl_nothrow
      {
        return std::error_condition(ev, *this);
      }
      bool equivalent(int code, const std::error_condition& condition) const __ntl_nothrow
      {
        return default_error_condition(code) == condition;
      }

      bool equivalent(const std::error_code& code, int condition) const __ntl_nothrow
      {
  ...
  inline std::error_code make_error_code(win32error st)
  {
    return std::error_code(static_cast<int>(st), win32_category());
  }

  inline std::error_condition make_error_condition(win32error st)
  {
    return std::error_condition(static_cast<int>(st), win32_category());
  }
